...
class UTF8Formatter(logging.Formatter):
    """Custom formatter that ensures UTF-8 encoding for log messages"""
...
def setup_logging(
    ...
    logger.info(
        f"Logging configured. Level: {log_level}, Save to file: {save_to_file}"
    )
    if save_to_file:
        logger.info(f"Log file: {log_file}, Max size: {max_file_size/1024/1024:.1f}MB, Backups: {backup_count}")
